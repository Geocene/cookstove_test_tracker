...
#_lib = ctypes.windll.usbtc08
#print(time.clock())
if '32' in platform.architecture()[0]:
	cbw32 = ctypes.windll['C:/Program Files (x86)/Measurement Computing/DAQ/cbw32.dll']
else:
	cbw32 = ctypes.windll['C:/Program Files (x86)/Measurement Computing/DAQ/cbw64.dll']
#print(time.clock())


# Taken directly from cbw.h and changed to Python syntax
...
def CHECK(value):
	if value != 0:
		raise RuntimeError('CB.CHECK: %d' % value)

def AIn(board, channel, range):
	dataValue = ctypes.c_short()
	CHECK(cbw32.cbAIn(board, channel, range, BYREF(dataValue)))
	return dataValue.value

def AInScan(board, loChan, hiChan, count, rate, range, memHandle, options):
	rate2 = ctypes.c_long()
	rate2.value = rate
	CHECK(cbw32.cbAInScan(board, loChan, hiChan, count, BYREF(rate2), range, memHandle, options))
	return rate2.value

def AOut(board, channel, range, dataValue):
	CHECK(cbw32.cbAOut(board, channel, range, dataValue))

def DConfigPort(board, portNum, direction):
	CHECK(cbw32.cbDConfigPort(board, portNum, direction))

def DIn(board, portNum):
	dataValue = ctypes.c_short()
	CHECK(cbw32.cbDIn(board, portNum, BYREF(dataValue)))
	return dataValue.value

def DOut(board, portNum, dataValue):
	CHECK(cbw32.DOut(board, portNum, dataValue))

def DBitIn(board, portNum, bitNum):
	bitValue = ctypes.c_short()
	CHECK(cbw32.cbDBitIn(board, portNum, bitNum, BYREF(bitValue)))
	return bitValue.value

def DBitOut(board, portNum, bitNum, bitValue):
	CHECK(cbw32.cbDBitOut(board, portNum, bitNum, bitValue))

def GetConfig(infoType, board, devNum, configItem):
	configVal = ctypes.c_int()
	CHECK(cbw32.cbGetConfig(infoType, board, devNum, configItem, BYREF(configVal)))
	return configVal.value

def InByte(board, portNum):
	return cbw32.cbInByte(board, portNum)

def OutByte(board, portNum, portVal):
	cbw32.cbOutByte(board, portNum, portVal)

def TIn(board, chan, scale=CELSIUS, options=FILTER):
	temp = ctypes.c_float()
	status = cbw32.cbTIn(board, chan, scale, BYREF(temp), options)
	if status == OPENCONNECTION: status = 0
	CHECK(status)
	return temp.value

def TInScan(board, loChan, hiChan, scale=CELSIUS, options=FILTER):
	n = hiChan-loChan+1
	temps = (ctypes.c_float*n)()
	status = cbw32.cbTInScan(board, loChan, hiChan, scale, BYREF(temps), options)
	if status == OPENCONNECTION: status = 0
	CHECK(status)
	return temps[:]

def WinBufAlloc(numPoints):
	handle = cbw32.cbWinBufAlloc(numPoints)
	assert handle != 0
	return handle

def WinBufFree(handle):
	CHECK(cbw32.cbWinBufFree(handle))

#TODO: What should this actually return, probably a numpy object directly
def WinBufToArray(handle, firstPoint, count):
#	buffer = (ctypes.c_ushort * count)()
	buffer = (ctypes.c_char * (2*count))()
#	CHECK(cbw32.cbWinBufToArray(handle, BYREF(buffer), firstPoint, count))
	return buffer[:]

#print(time.clock())
if __name__ == "__main__":
#	print cbAIn(0, 0, BIP5VOLTS)
	#hBuf = WinBufAlloc(1000)
	#AInScan(board=0, loChan=1, hiChan=1, count=1000, rate=500, range=BIP5VOLTS, memHandle=hBuf, options=0)
	#x =  WinBufToArray(hBuf, 0, 1000)[0:50]
#	for chan in range(0,4):
#		print(TIn(0, chan, CELSIUS, FILTER))
#	print(time.clock())
#	print(TInScan(0, 0, 3, CELSIUS, FILTER))
#	print(time.clock())
	s = raw_input("Press any key to unload Measurement Computing DLL")
